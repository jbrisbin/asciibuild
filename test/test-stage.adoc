...
This defines the environment this build will run in. This always gets run, no matter what `ASCIIBUILD_SECTION` says.

.Alpine Build
[[alpine]]
[source,Dockerfile]
[asciibuild,Dockerfile,image={image}-alpine,build_opts="",run={run}]
----
FROM alpine
...
[[ubuntu]]
[source,Dockerfile]
[asciibuild,Dockerfile,image={image}-ubuntu,build_opts="",run={run}]
----
FROM ubuntu
...
This defines what happens after the build. This always gets run, no matter what `ASCIIBUILD_SECTION` says.
